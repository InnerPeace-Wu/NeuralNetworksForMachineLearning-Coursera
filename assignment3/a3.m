...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  num_batch = size(class_prob, 2);
  output_delta = class_prob .- data.targets;
  in_to_hi_delta = model.hid_to_class' * output_delta .* (hid_output .* (1 .- hid_output)) ;
  #fprintf('size of in_to_hi_delta: %d', size(in_to_hi_delta));
  ret.input_to_hid = in_to_hi_delta * data.inputs' / num_batch + wd_coefficient * model.input_to_hid ;
  ret.hid_to_class = (output_delta) * hid_output' / num_batch + wd_coefficient * model.hid_to_class ;
end
